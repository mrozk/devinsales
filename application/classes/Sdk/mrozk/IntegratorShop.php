...
class IntegratorShop extends \DDelivery\Adapter\PluginFilters
{
    ...
    public function onFinishResultReturn( $order, $resultArray ){
        ...
        $resultArray['comment'] .= (', ' . $point['delivery_company_name']);
        if($order->type == \DDelivery\Sdk\DDeliverySDK::TYPE_SELF ){
            $resultArray['comment'] .= ( ', ' . $point['name'] . ', ID точки - ' . $point['_id'] );
        }
        $resultArray['street'] = '' ;
        $resultArray['house'] = '' ;
        $resultArray['corp'] = '' ;
    ...
    public function  processClientPrice(  $order, $price, $orderType, $companyArray  ){
        ...
        if ( $order->addField1 == 52 ){
           if( ( ($orderType == \DDelivery\Sdk\DDeliverySDK::TYPE_SELF) && ($order->type != \DDelivery\Sdk\DDeliverySDK::TYPE_COURIER)  ) || ($order->type == \DDelivery\Sdk\DDeliverySDK::TYPE_SELF)){
               if( $companyArray['delivery_company'] == 37 ){
                   return 0;
               }
           }

           if( ( ($orderType == \DDelivery\Sdk\DDeliverySDK::TYPE_COURIER) && ( $order->type != \DDelivery\Sdk\DDeliverySDK::TYPE_SELF )) || ($order->type == \DDelivery\Sdk\DDeliverySDK::TYPE_COURIER) ){

               if( $companyArray['delivery_company'] == 48 && $order->amount >= 2500 ){
                   return 0;
               }else{
                   if( $this->isPayPickup() ){
    ...
    public function getEmptyCompanyError( $order ){
        if( $order->addField1 == 52 ){
            return '<p>Извините, этот способ доставки не доступен для выбранного города.</p>
                    <p>Попробуйте выбрать другой способ доставки.</p>
                    <p>Или свяжитесь с нами, для уточнения вариантов доставки в Ваш город.</p>
                    <p>8 (800) 775 76 82 (по России бесплатно)</p>
                    <br />
                    <br />
                    <p>*"Доставка Почтой России, возможна во все регионы РФ, без исключения."</p>
